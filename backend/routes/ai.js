...
router.post('/video-prompt', async (req, res) => {
  ...
  const system = isCreator
    ? `You are a TikTok/Reels content director writing Google Gemini Veo prompts for short-form UGC (user-generated-content) product videos. The video MUST show a real-looking person ON CAMERA holding or showcasing the named product — selfie style or 3/4 angle, natural daylight or soft ring-light, presenter smiles/reacts to camera, product held clearly in frame near the face or chest. Authentic, spoken-style, social-media vibe. Write ONE English Veo prompt (~70-100 words) describing: the presenter (age range, expression, framing), how they hold/show the product, camera setup (selfie or POV), lighting, background, mood. Do NOT write a polished studio commercial — this is UGC. NO commentary, NO JSON, NO quotes — just the prompt itself.${hasReferenceImage ? ' A reference image of the exact product will be supplied to Veo — tell Veo to keep the product design/colors/details faithful to the reference.' : ''}`
    : isProductShot
    ? `You are a commercial director writing Google Gemini Veo prompts for short-form PRODUCT SALES videos (Reels / TikTok / Shorts). The named product MUST be the hero of the shot — visible, on-camera, beautifully lit, the clear subject. Write ONE polished English video prompt (~60-90 words) describing: hero product front-and-center, camera movement (slow push-in, orbit, top-down reveal, etc.), product-beauty lighting (rim light, soft key, dramatic shadow), composition (rule of thirds, centered hero, macro detail), mood, and color palette that complements the product.${styleGuide ? ' Style: ' + styleGuide.rule : ''} NO commentary, NO JSON, NO quotes — just the prompt itself.${hasReferenceImage ? ' A reference image of the exact product will be supplied to Veo, so describe the product faithfully and tell Veo to keep its design, colors, and details consistent with the reference.' : ''}`
    : `You are a cinematographer writing prompts for Google Gemini Veo (text-to-video AI). Output ONE polished English video prompt (~60-90 words). Describe: camera movement, composition, lighting, mood, color palette, key subjects.${styleGuide ? ' Style: ' + styleGuide.rule : ''} NO commentary, NO JSON, NO quotes — just the prompt itself.`;
...
${hasReferenceImage ? 'A reference image of the hero product is supplied — match its appearance exactly.' : ''}
${styleGuide ? 'Visual style: ' + styleGuide.label + ' — ' + styleGuide.rule : (style ? 'Visual style: ' + style : '')}
Video concept / hook: ${topic}
Duration: ~${duration_sec}s

Write ONE ${isCreator ? 'UGC creator-style' : isProductShot ? 'product-sales' : 'cinematic'} English Veo prompt that ${isCreator ? 'shows a real person on camera holding the product' : isProductShot ? 'sells the hero product' : 'brings this concept to life'}.`;
  try {
