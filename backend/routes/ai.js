...
async function generateImageGoogleDirect(prompt) {
  ...
  const fullId = OPENROUTER_IMAGE_MODEL || 'google/gemini-2.5-flash-image-preview';
...
async function generateImageOpenAIDirect(prompt) {
  const apiKey = process.env.OPENAI_API_KEY;
  if (!apiKey) throw new Error('OPENAI_API_KEY not set — add ใน .env');

  // map model id: openai/gpt-image-1 → gpt-image-1
  const model = (OPENROUTER_IMAGE_MODEL || '').replace(/^openai\//, '') || 'gpt-image-1';
...
router.post('/veo-generate', async (req, res) => {
  ...
  try {
    ...
    const d = await r.json();
    if (!r.ok || !d.name) {
      return res.status(500).json({ error: d.error?.message || `Veo submit ${r.status}`, detail: d });
    }
    res.json({ ok: true, operation: d.name, model: VEO_MODEL });
  } catch (e) {
