...
✅ หลากหลาย kind — กระจาย ${n} หัวข้อให้ทั้ง 6 ประเภทมีอย่างน้อย ${Math.max(2, Math.floor(n/8))} หัวข้อต่อประเภท:
   - knowledge (ให้ความรู้): ${Math.ceil(n*0.20)} หัวข้อ
   - promo (ขายตรง): ${Math.ceil(n*0.15)} หัวข้อ
   - review (รีวิว/case study): ${Math.ceil(n*0.15)} หัวข้อ
   - story (เล่าเรื่อง): ${Math.ceil(n*0.15)} หัวข้อ
   - tip (Tip ใช้ได้จริง): ${Math.ceil(n*0.20)} หัวข้อ
   - engage (ถาม-ตอบ ชวนคอมเมนต์): ${Math.ceil(n*0.15)} หัวข้อ
✅ หัวข้อต้อง**แตกต่างกัน 100%** — ไม่ซ้ำมุม, ไม่ซ้ำคำ keyword, ไม่ซ้ำ angle. กระจายให้กว้างที่สุด

แต่ละหัวข้อเลือก "kind" 1 อย่างจาก: knowledge, promo, review, story, tip, engage

ตอบเป็น JSON array เท่านั้น (ไม่มี markdown, ไม่มีคำอธิบาย, ไม่มี \`\`\`) — exactly ${n} elements:
